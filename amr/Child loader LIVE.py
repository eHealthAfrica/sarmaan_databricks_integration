...
TARGET_TABLE = "amr_child_information"
...
def existing_ids(engine, id_col="concatenated_id"):
    ...
    loader on the same staging file doesn't create duplicate rows. A missing
    table means nothing is loaded yet; any other error stops the load rather
    than appending without the check."""
    if not inspect(engine).has_table(TARGET_TABLE, schema=TARGET_SCHEMA):
        print(f"[{ts()}] {TARGET_SCHEMA}.{TARGET_TABLE} does not exist yet - nothing to check against.")
        return set()
...
def load_to_postgres():
    print(f"[{ts()}] PHASE 2: Starting Child Data Load...")

    df = pd.read_excel(STAGING_EXCEL, dtype=str)
    print(f"[{ts()}] Loaded {len(df)} rows from Excel.")

    cols_to_drop = ['household_code_pull', 'mother_code_pull']
    existing_drops = [c for c in cols_to_drop if c in df.columns]
    if existing_drops:
        df = df.drop(columns=existing_drops)
        print(f"[{ts()}] Dropped verification columns: {existing_drops}")
    ...
    print(f"[{ts()}] Appending {len(df)} rows to {TARGET_SCHEMA}.{TARGET_TABLE}...")
    df.to_sql(name=TARGET_TABLE, con=engine, schema=TARGET_SCHEMA, if_exists="append", index=False)
    print(f"[{ts()}] SUCCESS: Child data ingestion complete.")
